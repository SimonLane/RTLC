...
def build_preview(folderLocation,Cs, filename):
    ...
    filename = 'Scan%s.tif' %filename
    zpmc.save(filename,'TIFF')
    return filename


def get_info_from_metadata(folderLocation):
    ...
    a = metadata.find('LaserLine" Value="488" />')
    b = metadata.find('IntensityDev" Value="',a)
    c = metadata.find('" />',b)
    L488 = float(metadata[b+21:c])
    
    a = metadata.find('LaserLine" Value="514" />')
    b = metadata.find('IntensityDev" Value="',a)
    c = metadata.find('" />',b)
    L514 = float(metadata[b+21:c])

    a = metadata.find('LaserLine" Value="594" />')
    b = metadata.find('IntensityDev" Value="',a)
    c = metadata.find('" />',b)
    L594 = float(metadata[b+21:c])

    a = metadata.find('PinholeAiry" Value="')
    b = metadata.find('" />',a)
    pinhole = (metadata[a+20:b])
    
    return voxelx, voxelz, zoom, L488, L514, L594, pinhole
